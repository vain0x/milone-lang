...
let pToLoc (_, _, y, x) = y, x

let pIsAligned (_, locX: int) (_, _, _, x: int) = x = locX

let pIsDeeper (_, locX: int) (_, _, _, x: int) = x > locX
...
let lowerFields fields =
  ...
  | (ident, Yaml.Str ty) :: fields -> (ident, ty) :: lowerFields fields
...
let lower (yaml: Yaml) =
  match yaml with
  | Yaml.Table entries -> lowerRecords entries
...
let generateGetters ident fields acc =
  let rec genPats i j fields acc =
    ...
    | (fieldIdent, _) :: fields ->
        ...
        let ident =
          if i = j then toLowerCamel fieldIdent else "_"
