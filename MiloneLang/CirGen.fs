...
let genCirWithSplit (decls, mirCtx: MirCtx): bool * CDecl list * CDecl list =
  let ctx = ofMirCtx mirCtx
  ...
  let stdDecls, userDecls =
    List.fold
      (fun (stdDecls, userDecls) decl ->
        let loc =
          match decl with
        ...
        if isStdDoc docId then
          decl :: stdDecls, userDecls
        else
          stdDecls, decl :: userDecls)
      ([], [])
      decls

  let stdOk, stdDecls =
    let ctx = cgDecls ctx (List.rev stdDecls)
    let ok, ctx = genLogs ctx
    ok, ctx.Decls |> List.rev

  let userOk, userDecls =
    let ctx = cgDecls ctx (List.rev userDecls)
    let ok, ctx = genLogs ctx
    ok, ctx.Decls |> List.rev

  stdOk && userOk, stdDecls, userDecls
