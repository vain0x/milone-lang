...
let private findFromNode (keyCompare: 'K -> 'K -> int) key node : 'T option =
  let rec go node =
    match node with
    | E -> None
...
let private insertNode (keyCompare: 'K -> 'K -> int) (newKey: 'K) (newValue: 'T) node : Node<'K, 'T> =
...
let private removeNode (keyCompare: 'K -> 'K -> int) key node : 'T option * Node<'K, 'T> =
  let rec go node =
    match node with
    | E -> None, E
...
let private mapNode (f: 'K -> 'T -> 'U) (node: Node<'K, 'T>) : Node<'K, 'U> =
  let rec go node =
    match node with
    | E -> E
...
let private foldNode (folder: 'S -> 'K -> 'T -> 'S) (state: 'S) (node: Node<'K, 'T>) : 'S =
...
let private mapFoldNode (folder: 'S -> 'K -> 'T -> 'U * 'S) (state: 'S) (node: Node<'K, 'T>) : Node<'K, 'U> * 'S =
  let rec go state node =
    match node with
    | E -> E, state
...
let ofList keyCompare (assoc: ('K * 'T) list) : TreeMap<'K, 'T> =
  let node =
    assoc
    |> List.fold (fun node (k, v) -> insertNode keyCompare k v node) E
