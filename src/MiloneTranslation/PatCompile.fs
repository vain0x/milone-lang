...
module rec MiloneTranslation.PatternCompile
...
let private litCompare l r = todo ()
...
let private patToVars pat =
  let rec go pat acc =
    ...
    | HVarPat (_, varSerial, ty, loc) -> (varSerial, ty, loc) :: acc

    | HNodePat (_, args, _, _) -> args |> List.fold (fun acc pat -> go pat acc) acc

    | HAsPat (bodyPat, varSerial, loc) ->
        let ty = patToTy bodyPat
        go bodyPat ((varSerial, ty, loc) :: acc)

    | HOrPat (l, _, _) -> go l acc
...
/// Such pattern must be elaborated on construction.
///
...
type private PPat =
  | WildcardPat
  | TuplePat of PPatEx list

  | LitPat of Lit

  | NilPat
  | ConsPat of PPatEx * PPatEx

  | NonePat
  | SomePat of PPatEx

  | ConstVariantPat of VariantSerial
  | FunVariantPat of FunVariant * PPatEx

type private PPatEx = PPat * VarSerial option
...
type private PClause = ClauseIndex * PPatEx list * bool * (PTerm -> PTerm)
...
let private matchOnLitTy (cond: PCond) (clauses: PClause list) : AssocMap<Lit, PClause list> * PClause list =
  let rec go (clauses: PClause list) =
    match clauses with
    | [] -> TMap.empty (fun l r -> litCompare r l), []
    ...
    | (ci, pats, isGuarded, hole) :: clauses ->
        ...
        | (LitPat lit, varOpt) :: pats ->
            let caseMap =
              ...
              cases
              |> TMap.add lit ((ci, pats, isGuarded, hole) :: clauses)
...
let private doCompile (conds: (PPath * PTy) list) (clauses: PClause list) : bool * PTerm =
  ...
  | (cond, PLitTy) :: conds ->
      ...
      // See the guard above.
      assert (caseMap |> TMap.isEmpty |> not)
...
let private pcTy (ctx: PcCtx) ty : PTy =
  ...
  | TupleTk, _ -> tyArgs |> List.map (pcTy ctx) |> PTupleTy

  | ListTk, [ itemTy ] -> itemTy |> pcTy ctx |> PListTy

  | _ -> PUnknownTy

let private pcPat (ctx: PcCtx) pat : PPatEx =
  let todo () = failwithf "unimplemented. %A" pat

  match pat with
  | HLitPat (lit, _) -> LitPat lit, None

  | HNodePat (HNilPN, _, _, _) -> NilPat, None
  | HNodePat (HNonePN, _, _, _) -> todo ()

  | HNodePat (HSomeAppPN, [ payload ], _, _) -> SomePat(pcPat ctx payload), None
  | HNodePat (HSomeAppPN, _, _, _) -> unreachable ()

  | HDiscardPat _ -> WildcardPat, None

  | HVarPat (_, varSerial, _, _) -> WildcardPat, Some varSerial

  | HVariantPat _ -> todo ()

  | HNodePat (HConsPN, [ l; r ], _, _) ->
      let l = l |> pcPat ctx
      let r = r |> pcPat ctx
      ConsPat(l, r), None

  | HNodePat (HConsPN, _, _, _) -> unreachable ()

  | HNodePat (HTuplePN, itemPats, _, _) ->
      let itemPats = itemPats |> List.map (pcPat ctx)
      TuplePat itemPats, None

  | HNodePat (HVariantAppPN _, _, _, _) -> todo ()

  | HNodePat (HBoxPN, _, _, _)
  | HNodePat (HAbortPN, _, _, _) -> todo ()
  ...
  | HOrPat _ -> unreachable () // Resolved by patNormalize.

let private patCanCompile (ctx: PcCtx) pat =
  match pat with
  | HNodePat (HNonePN, _, _, _)
  | HNodePat (HSomeAppPN, _, _, _)
  | HNodePat (HBoxPN, _, _, _)
  | HNodePat (HAbortPN, _, _, _)
  | HVariantPat _ -> false

  | HLitPat _
  | HNodePat (HNilPN, _, _, _)
  | HDiscardPat _
  | HVarPat _ -> true

  | HNodePat (HConsPN, args, _, _) -> args |> List.forall (patCanCompile ctx)

  | HNodePat (HTuplePN, itemPats, _, _) -> itemPats |> List.forall (patCanCompile ctx)

  | HNodePat (HVariantAppPN _, _, _, _) -> false

  | HAsPat (bodyPat, _, _) -> patCanCompile ctx bodyPat

  | HOrPat _
  | HNodePat (HBoxPN, _, _, _)
  | HNodePat (HAbortPN, _, _, _) -> false
...
let private counterEmpty comparer : Counter<_> = TMap.empty comparer
  ...
  let rec go term =
    ...
    | PLetTerm (varSerial, init, next) ->
        let pat =
          ...
          HVarPat(PrivateVis, varSerial, varDef.Ty, varDef.Loc)

        let init = useCond init
        let next = go next

        HLetValExpr(pat, init, next, targetTy, loc)
    ...
    | PMatchListTerm (cond, nilCl, consCl) ->
        ...
        let arms =
          ...
          [ HNodePat(HNilPN, [], itemTy, loc), guard, go nilCl
...
let private pcExpr (expr, ctx: PcCtx) =
  ...
  | HMatchExpr (cond, arms, ty, loc) ->
      ...
      if not isSupported then
        ...
        let clauses : PClause list =
          arms
          |> List.mapi
               (fun (ci: ClauseIndex) (pat, guard, _body) ->
        ...
        let _, bodies, hole, ctx =
          arms
          |> List.fold
               (fun (i, bodies, hole, ctx: PcCtx) (pat, _guard, body) ->
                 ...
                 let arity, armBody, funTy, hole, ctx =
                   if args |> List.isEmpty then
                     let hole =
                       fun next ->
                         let unitPat = hpUnit loc
                         hole (HLetFunExpr(funSerial, NotRec, PrivateVis, [ unitPat ], body, next, ty, loc))
                     ...
                     let hole =
                       fun next ->
                         let argPats =
                           args
                           |> List.map (fun (serial, ty, loc) -> HVarPat(PrivateVis, serial, ty, loc))

                         hole (HLetFunExpr(funSerial, NotRec, PrivateVis, argPats, body, next, ty, loc))
                     ...
                     let armBody =
                       ...
                       args
                       |> List.fold
                            (fun f (varSerial, argTy, loc) ->
                              let tTy = f |> exprToTy |> tyToFunResult
                              hxApp f (HVarExpr(varSerial, argTy, loc)) tTy loc)
                            funExpr

                     arity, armBody, funTy, hole, ctx

                 let bodies = bodies |> TMap.add i armBody

                 let ctx =
                   let funDef : FunDef =
                     { Name = "clause_" + string (i + 1)
                       Arity = arity
                       Ty = TyScheme([], funTy)
                       Abi = MiloneAbi
                 ...
                 i + 1, bodies, hole, ctx)
               (0, TMap.empty compare, id, ctx)

        let matchExpr =
          let _, guards =
            arms
            |> List.fold
                 (fun (i, guards) (_, guard, _) ->
                   let guards =
                     if guard |> hxIsAlwaysTrue then
                       guards
                     else
                       guards |> TMap.add i guard

                   i + 1, guards)
                 (0, TMap.empty compare)

          // let _, bodies =
          //   arms
          //   |> List.fold (fun (i, bodies) (_, _, body) ->
          //        let bodies = bodies |> TMap.add i body
          //        i + 1, bodies) (0, TMap.empty compare)
  ...
  | HRecordExpr (baseOpt, fields, ty, loc) ->
      (fun () ->
        ...
        let fields, ctx =
          (fields, ctx)
          |> stMap
               (fun ((fieldIdent, fieldExpr, fieldLoc), ctx) ->
  ...
  | HNodeExpr (kind, args, ty, loc) ->
      (fun () ->
        let args, ctx = (args, ctx) |> stMap pcExpr
        HNodeExpr(kind, args, ty, loc), ctx)
        ()

  | HBlockExpr (stmts, last) ->
      (fun () ->
        let stmts, ctx = (stmts, ctx) |> stMap pcExpr
        let last, ctx = (last, ctx) |> pcExpr
...
let patCompile (expr: HExpr, tyCtx: TyCtx) =
