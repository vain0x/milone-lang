...
let private xcRval (rx: Rx) (rval: XRval) =
  ...
  | XBinaryRval (binary, l, r, _) ->
    ...
    | XScalarSubBinary -> CBinaryExpr(CSubtractBinary, l, r)
    | XScalarMulBinary -> CBinaryExpr(CMultiplyBinary, l, r)
    | XScalarDivBinary -> CBinaryExpr(CDivideBinary, l, r)
...
let private xcBody (rx: Rx) (wx: Wx) bodyId : Wx =
  ...
  let wx =
    ...
    let funDecl =
      ...
      CFunDecl(cBodyToName bodyId, args, xcTy bodyDef.ResultTy, stmts, isNoReturn)
