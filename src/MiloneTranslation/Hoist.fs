...
let private hoistExprLetFunForNonMainFun (expr, ctx) : HExpr * HoistCtx =
  let callee, isRec, args, body, next, loc =
    match expr with
    | HLetFunExpr (callee, isRec, args, body, next, _, loc) -> callee, isRec, args, body, next, loc
    | _ -> unreachable ()
  ...
  let ctx =
    ctx
    |> addDecl (HLetFunExpr(callee, isRec, args, body, hxDummy, tyUnit, loc))
...
let private isMainFun expr (ctx: HoistCtx) : bool =
  match expr with
  | HLetFunExpr (callee, _, _, _, _, _, _) ->
