  ...
  | None ->
      ...
      let ctx =
        ...
        |> cirCtxWithTyEnv
             (ctx
              |> cirCtxGetTyEnv
              |> mapAdd funTy (TyInstance.Declared, funStructTy))
...
let cirCtxAddFunDecl (ctx: CirCtx) sTy tTy =
  let funTy = tyFun sTy tTy
  match ctx |> cirCtxGetTyEnv |> mapTryFind funTy with
  | Some (TyInstance.Defined, ty) -> ty, ctx
  ...
  | _ ->
      ...
      let ctx =
        ...
        |> cirCtxWithTyEnv
             (ctx
              |> cirCtxGetTyEnv
              |> mapAdd funTy (TyInstance.Defined, selfTy))
  ...
  | None ->
      ...
      let ctx =
        ...
        |> cirCtxWithTyEnv
             (ctx
              |> cirCtxGetTyEnv
              |> mapAdd listTy (TyInstance.Declared, selfTy))
  ...
  | _ ->
      ...
      let ctx: CirCtx =
        ...
        |> cirCtxWithTyEnv
             (ctx
              |> cirCtxGetTyEnv
              |> mapAdd listTy (TyInstance.Defined, selfTy))
  ...
  | None ->
      ...
      let ctx =
        ...
        |> cirCtxWithTyEnv
             (ctx
              |> cirCtxGetTyEnv
              |> mapAdd tupleTy (TyInstance.Declared, selfTy))
  ...
  | _ ->
      ...
      let ctx: CirCtx =
        ...
        |> cirCtxWithTyEnv
             (ctx
              |> cirCtxGetTyEnv
              |> mapAdd tupleTy (TyInstance.Defined, selfTy))
  ...
  | None ->
      ...
      let ctx =
        ...
        |> cirCtxWithTyEnv
             (ctx
              |> cirCtxGetTyEnv
              |> mapAdd unionTyRef (TyInstance.Declared, selfTy))
  ...
  | _ ->
      ...
      let variants, ctx =
        ...
        |> stFlatMap (fun ((_, serial, hasPayload, payloadTy), acc, ctx) ->
             if hasPayload then
               ...
               (cirCtxUniqueName ctx serial, CTy.Ptr payloadTy)
               :: acc,
               ctx
             else
               acc, ctx)
      ...
      let ctx =
        ...
        |> cirCtxWithTyEnv
             (ctx
              |> cirCtxGetTyEnv
              |> mapAdd unionTyRef (TyInstance.Defined, selfTy))
...
let cirCtxConvertTyIncomplete (ctx: CirCtx) (ty: Ty): CTy * CirCtx =
  ...
  | Ty.Con (TyCon.Bool, _)
  | Ty.Con (TyCon.Int, _)
  | Ty.Con (TyCon.Tuple, []) -> CTy.Int, ctx

  | Ty.Con (TyCon.Char, _) -> CTy.Char, ctx

  | Ty.Con (TyCon.Str, _) -> CTy.Struct "String", ctx

  | Ty.Meta _ // FIXME: Unresolved type variables are `obj` for now.
  | Ty.Con (TyCon.Obj, _) -> CTy.Ptr CTy.Void, ctx
  ...
  | Ty.Con (TyCon.Ref serial, _) ->
      ...
      | _ -> CTy.Void, cirCtxAddErr ctx "Unknown type reference" noLoc // FIXME: source location

  | _ -> CTy.Void, cirCtxAddErr ctx "error type" noLoc // FIXME: source location
...
let genExprUniOp ctx op arg ty _ =
  ...
  match op with
  | MUniOp.Not -> CExpr.Uni(CUniOp.Not, arg), ctx
  ...
  | MUniOp.Unbox ->
      ...
      let deref =
        CExpr.Uni(CUniOp.Deref, CExpr.Cast(arg, CTy.Ptr valTy))
  ...
  | MUniOp.ListIsEmpty -> CExpr.Uni(CUniOp.Not, arg), ctx
...
let genInitBox ctx serial arg =
  ...
  let ctx =
    cirCtxAddLetAllocStmt ctx temp (CTy.Ptr argTy) (CTy.Ptr CTy.Void) storageModifier

  // *(T*)p = t;
  let left =
    CExpr.Uni(CUniOp.Deref, CExpr.Cast(CExpr.Ref temp, CTy.Ptr argTy))
...
let genInitIndirect ctx serial payload ty =
  ...
  // *(T*)p = t;
  let left =
    CExpr.Uni(CUniOp.Deref, CExpr.Cast(CExpr.Ref varName, ptrTy))
