...
int main(int argc, char** argv) {
    ...
    milone_assert((str_compare((*(((struct String const*)(&(p_[0]))))), (struct String){.str = "hey", .len = 3}) == 0), (struct String){.str = "native_ptr_read/native_ptr_read.milone", .len = 38}, 9, 2);
    str_as_ptr_result = str_as_ptr((struct String){.str = "Hello!", .len = 6});
    s_ = str_as_ptr_result;
    milone_assert(((*(((char const*)(&(s_[4]))))) == 'o'), (struct String){.str = "native_ptr_read/native_ptr_read.milone", .len = 38}, 12, 2);
    return 0;
}
