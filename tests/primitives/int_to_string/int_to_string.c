...
int milone_main(void) {
    ...
    milone_assert((str_compare(call_2, (struct String){.str = "0", .len = 1}) == 0), 9, 2);
    struct String call_3 = str_of_int64((0 - 97));
    milone_assert((str_compare(call_3, (struct String){.str = "-97", .len = 3}) == 0), 10, 2);
    struct String call_4 = str_of_char('a');
    milone_assert((str_compare(call_4, (struct String){.str = "a", .len = 1}) == 0), 11, 2);
    struct String call_5 = str_of_char('\0');
    milone_assert((str_compare(call_5, (struct String){.str = "", .len = 0}) == 0), 12, 2);
    milone_assert((str_compare((struct String){.str = "fix", .len = 3}, (struct String){.str = "fix", .len = 3}) == 0), 13, 2);
    return 0;
}
