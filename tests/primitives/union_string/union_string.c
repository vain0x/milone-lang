...
next_3:;
    ...
    payload_ = union_.MyUnion_CarryingOther;
    call_1 = string_of_int64(payload_);
    match_ = string_add(string_add((struct String){.ptr = "MyUnion.CarryingOther(", .len = 22}, call_1), (struct String){.ptr = ")", .len = 1});
    goto end_match_1;
next_4:;
...
int main(int argc, char **argv) {
    ...
    call_2 = MiloneDerive_UnionString_union0String((struct MyUnion_){.discriminant = MyUnion_UnitLike});
    if ((string_compare(call_2, (struct String){.ptr = "MyUnion.UnitLike", .len = 16}) != 0)) milone_assert_error("union_string/union_string.milone", 10, 2);
    variant_ = (struct MyUnion_){.discriminant = MyUnion_CarryingInt, .MyUnion_CarryingInt = 1};
    call_3 = MiloneDerive_UnionString_union0String(variant_);
    if ((string_compare(call_3, (struct String){.ptr = "MyUnion.CarryingInt(1)", .len = 22}) != 0)) milone_assert_error("union_string/union_string.milone", 11, 2);
    variant_1 = (struct MyUnion_){.discriminant = MyUnion_CarryingOther, .MyUnion_CarryingOther = 0};
    call_4 = MiloneDerive_UnionString_union0String(variant_1);
    if ((string_compare(call_4, (struct String){.ptr = "MyUnion.CarryingOther(0)", .len = 24}) != 0)) milone_assert_error("union_string/union_string.milone", 14, 2);
    return 0;
}
