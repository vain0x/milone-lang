...
else_3:;
    ...
    goto if_next_1;
if_next_1:;
    milone_assert(if_, (struct String){.ptr = "string_literal_raw/string_literal_raw.milone", .len = 44}, 7, 2);
    return;
}

void string_literal_raw_string_literal_raw_testNoEscape(void) {
    milone_assert((string_compare((struct String){.ptr = " \\\"\" ", .len = 5}, (struct String){.ptr = " \\\"\" ", .len = 5}) == 0), (struct String){.ptr = "string_literal_raw/string_literal_raw.milone", .len = 44}, 9, 22);
    return;
}
