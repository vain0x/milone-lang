...
int main() {
    ...
    milone_assert((str_cmp(john_.Name, (struct String){.str = "John Doe", .len = 8}) == 0), 10, 2);
    milone_assert((str_cmp(jane_.Name, (struct String){.str = "Jane Doe", .len = 8}) == 0), 11, 2);
    milone_assert((jane_.Age == 16), 12, 2);
    ...
    milone_assert((john_1.Age == 17), 15, 2);
    return 0;
}
