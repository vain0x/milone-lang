...
int main() {
    ...
    tuple_.t0 = 1;
    tuple_.t1 = NULL;
    void* box_ = milone_mem_alloc(1, sizeof(struct IntEven_ListTuple2));
    (*(((struct IntEven_ListTuple2*)box_))) = tuple_;
    struct Odd_ variant_ = (struct Odd_){.tag = Odd_, .Odd_ = box_};
    struct Odd_ one_ = variant_;
    struct Odd_List* some_ = milone_mem_alloc(1, sizeof(struct Odd_List));
    some_->head = one_;
    some_->tail = NULL;
    ...
    struct Even_List* some_1 = milone_mem_alloc(1, sizeof(struct Even_List));
    some_1->head = two_;
    some_1->tail = NULL;
    struct IntEven_ListTuple2 tuple_2;
    tuple_2.t0 = 3;
    tuple_2.t1 = some_1;
    void* box_1 = milone_mem_alloc(1, sizeof(struct IntEven_ListTuple2));
    (*(((struct IntEven_ListTuple2*)box_1))) = tuple_2;
    struct Odd_ variant_2 = (struct Odd_){.tag = Odd_, .Odd_ = box_1};
    struct Odd_ three_ = variant_2;
    struct Odd_List* some_2 = milone_mem_alloc(1, sizeof(struct Odd_List));
    some_2->head = three_;
    some_2->tail = NULL;
